...
\\\\
\todo{Talk about the sprint start to make the transition more smooth}
\tabref{tab:application_priorities_sprint_one} shows how the different applications were prioritized by the customers, with 0 being the highest priority. Please note that these different priorities do not reflect which applications the customers wants us to focus on, but rather which applications are important for them. So for instance, if an application has a high priority, but is almost ``done'' then it would require less man hours. \todo{Write that the priority is how highly the app is prioritized but the if the application is done it may not receive much work}
