...
We found the there was two common types of dialogs where we could provide a default implementation for convenience. The first being a simple notification dialog called \androidinline{GirafNotifyDialog} which includes a title, a message, and a button for closing the dialog. 

\todo{INSERT PICTURE OF DIALOG}

And the second a confirmation dialog called \androidinline{GirafConfirmDialog} which provides an extra confirmation button and the possibility to provide an action for that button through an interface which the calling activity must implement. 
