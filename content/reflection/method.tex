...
\todo[inline]{DONE-start(holm) xD - Reflekter over arbejde internt i gruppen}
\section{Group Synergy}
Overall the synergy regarding the teamwork in the the group have been great. This might be for the reason that we internally in the group have had the same ambitions from the start. Another reason for this could be that we knew each other well before we started this semester so we know each others pros and cons.
\\\\
Even though the groups was encouraged to use Scrum for the process of development this semester we started out by following this method strictly. However, over time we gradually stop using the methods from Scrum. We started by having stand up meetings everyday which ended in never having these meetings. However since the group only consists of four group members the purpose of scrum did not seem necessary internally in the group. For instance knowledge sharing was simply done ad hoc instead of using daily stand up meetings. However the group as a unit still followed the Scrum process on a multi project scale.
\todo[inline]{DONE-end(holm)}
